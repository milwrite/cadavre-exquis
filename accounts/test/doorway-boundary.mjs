...
{name:'cadavre',modules:true,compatibilityDate:'2026-09-06',scriptPath:resolve('../worker/dist-worker/index.js'),modulesRoot:resolve('..'),bindings:{CAIL_IDENTITY_JWKS:jwksJson,CADAVRE_DEFAULT_MODEL:'test/poetry',RELEASE:'boundary-test'},durableObjects:{STORE:{className:'CadavreStore',useSQLite:true}},assets:{directory:resolve('../worker/dist'),binding:'ASSETS',run_worker_first:true,routerConfig:{has_user_worker:true}},serviceBindings:{WORK_ACCOUNTS:{name:'accounts',entrypoint:'WorkerAccounts',props:cadavreManifest},ADMISSION_RESOLVER:admission,GATEWAY:gateway}},
...
try{
 ...
 const page=await call('/cadavre/');assert.equal(page.status,200,await page.clone().text());assert.match(await page.text(),/href="\/cadavre\/play\/"/);
 const clean=await call('/cadavre/ui/corpse.html?work=keep-me');assert.equal(clean.status,307);assert.equal(clean.headers.get('location'),origin+'/cadavre/ui/corpse?work=keep-me');
 const play=await call('/cadavre/play/?work=keep-me');assert.equal(play.status,200);assert.match(await play.text(),/id="brand"/);
